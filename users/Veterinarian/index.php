...
class Login {
  ...
  public function SessionCheck() {
    ...
    $businessIdQuery = "SELECT vet_id FROM veterinarian WHERE user_id = '{$_SESSION["user_id"]}'";
    $businessIdResult = mysqli_query($conn, $businessIdQuery);
    $businessIdRow = mysqli_fetch_assoc($businessIdResult);
    $businessId = isset($businessIdRow["vet_id"]) ? $businessIdRow["vet_id"] : null;

    // Store the business_id in the session
    $_SESSION["vet_id"] = $businessId;
  ...
  public function GetVetDetails() {
    ...
    $businessId = $_SESSION["vet_id"];
    // Run the query to fetch business details
    $businessQuery = "SELECT * FROM veterinarian WHERE vet_id = '$businessId'";
...
<script>
 ...
 <section class="glass">
      ...
      <form class="modern-form" action="update_vet.php" method="POST" >
      <?php
      if (isset($_SESSION['login'])) {
        $user = $login->GetVetDetails();
      ?>
  <div class="form-group">
  <label for="username">Username:</label>
  <input type="text" id="user_id" name="user_id" value="<?php echo $user["user_id"]; ?>" required style="display: none;">
  <input type="text" id="Susername" name="Susername" value="<?php echo isset($user["vet_name"]) ? $user["vet_name"] : ''; ?>" <?php echo isset($user["vet_name"]) ? 'required' : ''; ?>>
