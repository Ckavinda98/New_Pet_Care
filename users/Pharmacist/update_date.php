...
if (isset($_POST['prescription_id']) && isset($_POST['date'])) {
  // Get the prescription_id and date from the POST data
  $prescription_id = $_POST['prescription_id'];
  $date = $_POST['date'];

  // Call the updateDate function to update the prescription date
  updateDate($prescription_id, $date);
} else {
  echo "Invalid request.";
}

// Update the date of a prescription based on prescription_id
function updateDate($prescription_id, $date) {
  global $conn;

  // Perform the update query
  $query = "UPDATE prescriptions SET prescription_date = '$date' WHERE prescription_id = $prescription_id";
  $result = mysqli_query($conn, $query);

  // Check if the update was successful
  if ($result) {
    echo "Prescription Date updated successfully.";
  } else {
