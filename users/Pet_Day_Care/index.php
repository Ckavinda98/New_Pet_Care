...
class Login {
  ...
  $businessIdQuery = "SELECT day_care_id FROM pet_day_care WHERE user_id = '{$_SESSION["user_id"]}'";
  $businessIdResult = mysqli_query($conn, $businessIdQuery);
  $businessIdRow = mysqli_fetch_assoc($businessIdResult);
  $businessId = isset($businessIdRow["day_care_id"]) ? $businessIdRow["day_care_id"] : null;


  // Store the business_id in the session
  $_SESSION["day_care_id"] = $businessId;
  ...
  public function GetDayCareDetails() {
    ...
    $businessId = $_SESSION["day_care_id"];
    // Run the query to fetch business details
    $businessQuery = "SELECT * FROM pet_day_care WHERE day_care_id = '$businessId'";
...
<script>
 ...
 <section class="glass">
      ...
      <form class="modern-form" action="update_daycare.php" method="POST" >
      <?php
      if (isset($_SESSION['login'])) {
        $user = $login->GetDayCareDetails();
      ?>
  <div class="form-group">
  <label for="username">Username:</label>
  <input type="text" id="user_id" name="user_id" value="<?php echo $user["user_id"]; ?>" required style="display: none;">
  <input type="text" id="Susername" name="Susername" value="<?php echo isset($user["day_care_name"]) ? $user["day_care_name"] : ''; ?>" <?php echo isset($user["day_care_name"]) ? 'required' : ''; ?>>
...
<script>
  function updateService(userId) {
    ...
    // Perform the update using AJAX
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'update_daycare.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhr.onreadystatechange = function() {
      if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
        // Handle the response from the server
